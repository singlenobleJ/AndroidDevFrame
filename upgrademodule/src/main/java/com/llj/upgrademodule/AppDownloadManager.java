...
/**
 * https://juejin.im/post/5ad4ab7af265da239d49c8f9
 */
public class AppDownloadManager {
    ...
    public AppDownloadManager(Activity activity) {
        ...
        mDownloadManager = (DownloadManager) mWeakReference.get().getSystemService(Context.DOWNLOAD_SERVICE);
    ...
    public void downloadApk(String apkUrl, String title, String desc) {
        File apkFile = new File(mWeakReference.get().getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), "app_name.apk");
        if (apkFile.exists()) {
            apkFile.delete();
        }
        ...
        request.setDestinationInExternalFilesDir(mWeakReference.get(), Environment.DIRECTORY_DOWNLOADS, "app_name.apk");
    ...
    public void resume() {
        ...
        mWeakReference.get().getContentResolver().registerContentObserver(Uri.parse("content://downloads/my_downloads"), true,
                mDownLoadChangeObserver);
        // 注册广播，监听APK是否下载完成
        mWeakReference.get().registerReceiver(mDownloadReceiver, new IntentFilter(DownloadManager.ACTION_DOWNLOAD_COMPLETE));
    }
     ...
     */
    public void onPause() {
        mWeakReference.get().getContentResolver().unregisterContentObserver(mDownLoadChangeObserver);
        mWeakReference.get().unregisterReceiver(mDownloadReceiver);
    }
